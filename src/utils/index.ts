...
export function generatorMenu(routerMap: Array<any>) {
  return filterRouter(routerMap).flatMap((item) => {
    if (item.meta?.flatMenu && item.children?.length) {
      return generatorMenu(item.children).map((menu) => ({
        ...menu,
        icon: menu.icon || item.meta?.icon,
      }));
    }
